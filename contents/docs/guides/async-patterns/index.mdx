...
let results = await Promise.all(promises.map(p => go(p)));
...
// Simple GET request
let [data, error] = await goFetch('https://api.example.com/data');
if (error) {
...
let [data, error] = await goFetch<TransformedUser>('https://api.example.com/user', {
...
let [data, error] = await goFetch<unknown, CustomError>('https://api.example.com/data', {
  errorTransformer: (e) => ({
...
let [data, error] = await goFetch<TransformedResponse, ApiError>(
...
```typescript
// HTTP Errors
let [data, error] = await goFetch('https://api.example.com/notfound');
if (error?.includes('HTTP error! status: 404')) {
  console.log('Resource not found');
}

// Network Errors
let [data, error] = await goFetch('https://invalid-url');
if (error?.includes('Network error')) {
  console.log('Network is unreachable');
}

// Invalid JSON
let [data, error] = await goFetch('https://api.example.com/invalid-json');
...
2. **Type Your Transformers**
   ...
   let [data, error] = await goFetch<User>('https://api.example.com/user', {
