# Understanding go, goSync, and goAsync
Learn when and how to use each function variant effectively
...
const [result1, error1] = await go(operation);
...
const [result3, error3] = await goAsync(asyncOperation);
```
...
const [syncResult, syncError] = await go(() => {
  return "hello";
});

// Handling asynchronous operations
const [asyncResult, asyncError] = await go(async () => {
  return await fetch("https://api.example.com/data");
});

// Handling promises directly
const [promiseResult, promiseError] = await go(
...
const [num, err1] = await go(() => 42);
// num is number | null
// err1 is Error | null

const [str, err2] = await go(async () => "hello");
// str is string | null
// err2 is Error | null
```
...
```typescript
// Basic usage
const [result, error] = goSync(() => {
  return "hello";
});

// Type inference
const [num, err] = goSync(() => 42);
// num is number | null
// err is Error | null
